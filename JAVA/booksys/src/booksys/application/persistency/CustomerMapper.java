...
//		      + "' AND phoneNumber = '" + p + "'") ;
    	...
    	//휴대폰번호 복호화
    	c = getCustomer("SELECT oid, name, AES_DECRYPT(phoneNumber, SHA2('"+enc+"',512)) FROM Customer WHERE name = '" + n + "' and AES_DECRYPT(phoneNumber, SHA2('"+enc+"',512)) = '" + p + "'");
...
//    c = getCustomer("SELECT * FROM Customer WHERE oid ='" + oid + "'") ;
    	...
    	//휴대폰번호 복호화 코드
      c = getCustomer("SELECT oid, name, AES_DECRYPT(phoneNumber, SHA2('"+enc+"',512)) FROM Customer WHERE oid ='" + oid + "'");
      if (c != null) {
	addToCache(c) ;
      }
    }
...
//			       "VALUES ('" + name + "', '" + phone + "')") ;
	...
	//휴대폰 번호 암호화
	= stmt.executeUpdate("INSERT INTO Customer (name, phoneNumber)" + "VALUES ('" + name + "', AES_ENCRYPT('" + phone + "', SHA2('"+enc+"',512)))") ;
